...
- **The merge gate reads Gate A's input from the database, never from the caller (epic 44,
  ...
  Gate A reads the lens verdicts of the triage dispatch BOUND to the story — recorded in the
  new `story_stages.triage_dispatch_id` column (migration, no manual step) before its verdict is
  stored, first writer wins, so a second or late dispatch's verdict is refused `stale_stage` —
  or accepts a human's
