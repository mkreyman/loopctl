...
3. **Lineage comparison (primary)** when BOTH `implementer_dispatch_id` and
   ...
   (`Dispatches.lineage_shares_prefix?/2`, `lib/loopctl/dispatches.ex:563-568`) blocks, and the
...
(`lib/loopctl/dispatches.ex:517-527`) — the dispatch that minted the calling key. A key not
