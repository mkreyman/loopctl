...
| `DISPATCH_LEASE_GRACE_SECONDS` | `900` | Seconds past a dispatch's wall clock before the claim a PLACEMENT takes (`Loopctl.Delivery.Placement`, the dispatch driver and `place_dispatch`) is capped (#879). That claim's lease is `placed_at + wall_clock_seconds + this`, it is sent to the runner as the dispatch's `deadline_at`, and no renewal ever moves it later — so a killed session releases its story within minutes of its wall clock instead of holding it for `STORY_CLAIM_LEASE_SECONDS`. Every other claim keeps the global lease. It covers what runs BEFORE the runner's own wall clock starts (the push, the worktree setup). **Below `Loopctl.Runners.Capacity.release_grace_seconds/0` (300) THE APP REFUSES TO BOOT**, naming both values: a claim released before capacity presumes the session over lets the story be placed again under a live session. **Too SHORT** cuts a slow-starting session off before its wall clock runs out — an adopting runner stops it at `deadline_at` and control reclaims the story then; too long only delays reopening a story whose session is gone. Integer seconds; unset, blank or non-numeric leaves the default. Applies to claims placed after a restart; existing caps are not rewritten |
