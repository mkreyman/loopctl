defmodule Loopctl.Progress.ForceUnclaimResultCoverageTest do
  ...
  @moduledoc """
  846.8 AC-5. `Progress.force_unclaim_story/3` builds a five-step `Ecto.Multi` and matched its
  ...
  defp uncovered(multi, source) do
    handled = handled_names(source)
  ...
  defp multi_steps, do: steps_of(real_multi())
  ...
  defp steps_of(multi), do: multi |> Ecto.Multi.to_list() |> Enum.map(&elem(&1, 0))
  ...
  # `AdminRepo.transaction/1` inside `force_unclaim_story/3`. The two positions are the only
  # two that transaction offers:
  #
  #   * the second element of an error clause HEAD — `{:error, :lock, reason, _} ->`, a tuple
  #     of FOUR elements, which in the AST is `{:{}, _, [:error, :lock, _, _]}`;
