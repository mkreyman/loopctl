defmodule Loopctl.DeliveryGates.Measurement.RepoHistoryGitTest do
  ...
  2. The fixture is created under `Loopctl.RealTmpDir.path!/0` (the tmp dir with symlinks resolved, so it compares equal to git's physical toplevel on macOS), OUTSIDE the project tree, so a discovery
