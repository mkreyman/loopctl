defmodule LoopctlWeb.Plugs.DBErrorBackstopTest do
  ...
  describe "uncaught DB exception in ANY controller (AC-27.3.3 + AC-27.3.8)" do
    test "57014 is logged with the real SQLSTATE + request_id and re-raised sanitized",
      ...
      #
      # raise_uncaught/2 captures only what THIS process logged, so a concurrent async test's
      # own "SELECT" cannot redden these refutes, and every line of a multi-line entry is
      # checked, not only the one carrying the request id.
      refute log =~ "SELECT"
