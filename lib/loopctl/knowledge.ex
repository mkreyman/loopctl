defmodule Loopctl.Knowledge do
  ...
  *genuinely-empty* one (the corpus simply has fewer than `limit` eligible
  ...
  # corpus is missing OR embedded from text that has since changed; the enqueue decides
  # under an advisory lock whether a run is already in flight, so one is never doubled,
  # and refuses to re-drive a permanently-terminated one.
