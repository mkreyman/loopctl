defmodule Loopctl.ApiSpec.RunnerContract do
  ...
  | (1.15.0) A triage verdict message may carry `lens_verdicts` (`RunnerLensVerdict`, exactly one per lens, only beside a `verdict`, capped together by `RunnerLensVerdict.max_bytes/0`). Gate A reads them at merge instead of anything a merge caller supplies; a story whose triage sent none is refused at merge. RE-VENDOR to send them; a 1.14.0 holder keeps working and its stories are refused at merge | | | | |
  ...
  **A replay must carry the SAME identities its first copy did.** One that names a DIFFERENT
  ...
  - `triage_gating_reasons` (`Loopctl.DeliveryGates.GateA.gating_reason_codes/0`) — the
    ...
    **WHERE THEY GATE (since 1.15.0).** In the `escalation_reasons` of each
    ...
    own, at merge. The MERGED verdict's `escalation_reasons` still gate
          ...
          escalation_reasons: %Schema{
            ...
            description:
              "Why this needs a human. Meaningful when `outcome` is `escalate`. FREE-FORM: a " <>
                ...
                "`lens_verdicts` (since 1.15.0), matched as whole strings, at merge."
          },
      ...
      %{
        ...
        description:
          "One triage lens's own judgement (since 1.15.0), sent three at a time in " <>
            ...
            "with the verdict and Gate A reads them at merge instead of anything a merge " <>
            "caller supplies. SESSION-AUTHORED " <>
          ...
          lens_verdicts: %Schema{
            ...
            description:
              "The three triage lenses' own judgements (since 1.15.0), one per lens, each " <>
                ...
                "merge, so a story whose triage sent none is refused there: Gate A cannot " <>
                "be evaluated. At most " <>
