defmodule Loopctl.ApiSpec.RunnerContract do
  ...
  | (1.16.0) A dispatch control PLACES may carry `deadline_at` (`RunnerDispatch.deadline_at`): an instant the runner must END THE SESSION BY — the EARLIER of its own start + `wall_clock_seconds` and `deadline_at`. It is placed_at + `wall_clock_seconds` + `DISPATCH_LEASE_GRACE_SECONDS` (#879), so the time before the session starts comes out of the grace, not the wall clock; only a start-up longer than the grace shortens the session. loopctl's claim on the story never ends before it, so a runner that stops by it — even one cut off from control — never runs on a story placed again. OPTIONAL and additive: a 1.15.0 holder ignores it (undeclared keys are dropped) and nothing changes for it. RE-VENDOR to read it | | | | |
      ...
      %{
        ...
        properties: %{
          ...
          deadline_at: %Schema{
            ...
            format: :"date-time",
            description:
              "Since 1.16.0, OPTIONAL. A STOP BOUND: end the session by the EARLIER of " <>
                "your own start + `wall_clock_seconds` and this instant, whether or not " <>
                "control is reachable. It is placed_at + `wall_clock_seconds` + " <>
                "`DISPATCH_LEASE_GRACE_SECONDS`, so the time between placement and your " <>
                "start comes out of that grace, not out of `wall_clock_seconds`; only a " <>
                "start-up longer than the grace shortens the session. loopctl's claim on " <>
                "the story never ends before it — your acceptance may move the claim later, " <>
                "never earlier — so a session stopped by it never runs on a story loopctl " <>
                "has placed again. Present on a dispatch control PLACED under a claim; " <>
                "absent otherwise."
          },
