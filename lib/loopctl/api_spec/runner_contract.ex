defmodule Loopctl.ApiSpec.RunnerContract do
  ...
  | (1.16.0) A dispatch control PLACES may carry `deadline_at` (`RunnerDispatch.deadline_at`): the EARLIEST instant loopctl's claim on the story can end — placed_at + `wall_clock_seconds` + `DISPATCH_LEASE_GRACE_SECONDS` (#879). The runner budgets its session from its own `wall_clock_seconds`, exactly as before; its acceptance moves the claim's cap forward, so the claim is held until at least its `replied_at` + `wall_clock_seconds` + the grace, and no renewal moves it past that. OPTIONAL and additive: a 1.15.0 holder ignores it (undeclared keys are dropped) and nothing changes for it. RE-VENDOR to read it | | | | |
      ...
      %{
        ...
        properties: %{
          ...
          deadline_at: %Schema{
            ...
            format: :"date-time",
            description:
              "Since 1.16.0, OPTIONAL. The EARLIEST instant loopctl's claim on this story " <>
                "can end: placed_at + `wall_clock_seconds` + `DISPATCH_LEASE_GRACE_SECONDS`. " <>
                "It is not a stop time: budget the session from your own " <>
                "`wall_clock_seconds`, as without it. Your acceptance (`dispatch_reply` " <>
                "`accepted`) moves the claim forward, so the claim is held until at least " <>
                "your `replied_at` + `wall_clock_seconds` + that grace, and no renewal moves " <>
                "it past that. Present on a dispatch control PLACED under a claim; absent " <>
                "otherwise."
          },
