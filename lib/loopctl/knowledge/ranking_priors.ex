defmodule Loopctl.Knowledge.RankingPriors do
  ...
  #
  # Identified by BOTH the `hub` and `moc` tags, never `moc` alone — `moc` is also a
  # legitimate content tag (Market-On-Close), the same trap KnowledgeMocWorker calls
  # out when it looks hubs up by idempotency_key instead. Verified as an exact
  # discriminator before being relied on: on the published corpus the pair matches
  # 52 of 52 true hubs with zero false positives and zero missed.
  @moc_tags ["hub", "moc"]
  ...
  for dead doctrine (the `#{@kill_tag}` tag or `:superseded`) and for a MOC/index hub,
  else 1.0. Independent of the authority toggle.
  """
  ...
  def demotion_factor(result) do
    ...
    if @kill_tag in tags or status == :superseded or status == "superseded" or
         moc_hub?(tags) do
      @demote_factor
    else
      1.0
  ...
  @doc """
  Whether `tags` mark a generated MOC / "Index: <tag>" hub — navigation rather than an
  answer. Requires BOTH structural tags; `moc` alone is a legitimate content tag.
  """
  @spec moc_hub?([String.t()] | nil) :: boolean()
  def moc_hub?(tags) when is_list(tags), do: Enum.all?(@moc_tags, &(&1 in tags))
