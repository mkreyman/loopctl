defmodule LoopctlWeb.KnowledgeSearchJSON do
  ...
  def render_meta(meta) do
    ...
    # `hnsw.iterative_scan`. `"unavailable"` means the operator enabled it but the
    # capability probe fell closed, so the cross-tenant residual filter may have
    # under-returned — disclosed rather than left as a short result set the caller reads
    # as an empty corpus. `ann_iterative_scan_reason` accompanies that state only.
