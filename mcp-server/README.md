...
| `embedding_materialize_system_corpus` | Embed the shared SYSTEM-scoped corpus for THIS tenant at its active dimension with this tenant's own credential (system articles are keyword-only until then), and re-embed system articles edited since. Idempotent, batched: nothing missing or stale answers `already_materialized`, a run already queued or executing answers `in_flight`, and neither queues a job. 409 `materialization_terminal` when the tenant's latest job was discarded or cancelled and the key is agent-role. An orchestrator-or-higher key forces: a new job past a discarded, cancelled or executing one, and a job waiting out a backoff runs now. |
