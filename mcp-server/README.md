...
| `list_blocked_stories` | List stories blocked by an unverified dependency (their own, or their epic's) — the ones the dispatch driver skips and `place_dispatch` refuses `409 dependencies_not_met` (`GET /api/v1/stories/blocked`). Optional `project_id`, `page`, `page_size`. |
