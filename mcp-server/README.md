...
| `knowledge_create` | Create a new knowledge article. File findings, document patterns, or record decisions. Created as a **draft** (NOT visible to agents until published) — the response `note` says so. Pass `publish: true` to create-and-publish in one call (requires `LOOPCTL_ORCH_KEY`; routed through the orchestrator key). Optional: `category`, `tags`, `project_id`, `publish`. |
