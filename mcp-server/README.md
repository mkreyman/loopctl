...
| `thread_entry` | **Write an entry on a story's thread** (`POST /api/v1/stories/:id/thread/entries`): `message`, `review_requested`, `finding`, `fix` or `verdict`. A finding or verdict from the implementer is 409 `self_review_blocked`; a fix is the current claimant's only. A finding needs `checkpoint_id` and `severity` (and `introduced_by` after the first completed round); a fix needs `checkpoint_id` and `finding_ids`. A credential in the body is 422 `secret_blocked`. `principal` picks the key (agent, orchestrator, user). Idempotent per author on `idempotency_key`. |
