...
| `thread_get` | **Read a story's change thread** (`GET /api/v1/stories/:id/thread`, any role): its checkpoints and entries in `seq` order. Every entry body is untrusted text (`body_untrusted: true`). |
| `thread_checkpoint` | **Record a checkpoint on the story you hold** (`POST /api/v1/stories/:id/thread/checkpoints`, agent key): a pushed commit plus your reasoning in `note`. Refused 409 `not_claimant` / `stale_claim_epoch` unless you are the current claimant; idempotent on `commit_sha`; 422 on a malformed sha. Required: `story_id`, `claim_epoch`, `commit_sha`, `tree_sha`. |
| `thread_entry` | **Write an entry on a story's thread** (`POST /api/v1/stories/:id/thread/entries`): `message`, `review_requested`, `finding`, `fix` or `verdict`. A finding or verdict from the implementer or its dispatch chain is 409 `implementer_cannot_judge`, and from a key no dispatch minted it needs a human user key (else 409 `caller_lineage_required`). A fix is the current claimant's only, with its `claim_epoch`, carried by a checkpoint of the current claim newer than its findings. A finding needs `checkpoint_id` and `severity` (and `introduced_by` after the first completed round); a fix needs `checkpoint_id` and `finding_ids`. A credential in the body is 422 `secret_blocked`. `principal` picks the key (agent, orchestrator, user). Idempotent per author on `idempotency_key`; a different entry on the same key is 409 `idempotency_key_reused`, and keys starting `loopctl:` are reserved. `thread_get` pages entries with `after_seq` / `limit`. |
