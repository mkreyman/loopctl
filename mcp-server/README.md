...
expires, it is scoped to its tenant, and it is TYPED so a proposal for a reversible
op cannot be replayed as an irreversible one. Selectors too large to freeze fall
back to re-confirm-on-drift, which re-resolves and refuses on any change rather than
relaxing the guarantee. `knowledge_bulk_delete` is the surface this governs today,
