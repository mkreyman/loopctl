...
| `custody_claim` | The recorded **egress custody claim** for one article or memory row: the append-only sequence of per-operation postures (create, each embed, each re-embed, each classification/merge) with the endpoint resolved for THAT operation and its verdict, plus the aggregate. Rides the existing hash-chained audit log + signed tree heads — each entry carries a `chain_position` you can prove inclusion for at `GET /api/v1/audit/sth/{tenant_id}/inclusion/{position}`. THREE states, only one an attestation: `no_claim_recorded`, `claim_pending`, `claim_recorded` (`complete` / `incomplete`). Attests ONLY to the endpoints loopctl called on the paths in `coverage` — never to what those endpoints did afterwards. **Agent** key. |
| `custody_failures` | Custody posture entries whose chain append was DROPPED after exhausting retries. Surfaced rather than silently absent: each one degrades its row's claim to `incomplete`, because a missing claim must never read as a satisfied one. **Agent** key. |
