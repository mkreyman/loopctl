...
**Read `meta.outcome` before you believe an empty result.** Every retrieval and list
response on the knowledge, memory and corpus surfaces carries one of `success` | `empty` |
...
hidden behind a zero count. The MCP server prints a leading banner for the three classes
that need a remedy; `empty` and `success` are silent by design. Write tools carry no
`outcome`.
