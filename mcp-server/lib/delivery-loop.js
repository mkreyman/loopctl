 ...
 * that is false. `Placement.claimable/2` (`placement.ex:769-777`) wants `agent_status ==
 * :contracted` AND stage `queued`; the release leaves the story at `:pending`, and
 * `@valid_transitions` (`progress.ex:3916`) has `pending: :contracted` and nothing else — so
