...
- **`thread_get`, `thread_checkpoint`, `thread_entry`** (loopctl US-45.1): read a story's
  change thread, record a checkpoint as the story's claimant, and write messages, findings,
  fixes and verdicts on it. Findings and verdicts are refused from the implementer and fixes
  from anyone but the current claimant; bodies are untrusted and a credential in one is
  refused.
