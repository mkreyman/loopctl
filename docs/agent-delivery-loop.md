...
**Thread mode (epic 45, US-45.4).** An intake source set to `mode: thread` (`intake_source_update`) has no pull request. The gate judges the story's latest RECORDED checkpoint on the branch `loop/<story_id>` instead, needs no `pr_number`, and also refuses `empty_change` (the checkpoint's tree equals the base's, or no file changed), `checkpoint_tree_mismatch` and `no_checkpoint_recorded`. A branch naming a commit nobody reported is `head_moved` with reason `branch_head_unrecorded`, back to `implementing` like any moved head. An allow is recorded naming the checkpoint id and sha. When the control plane records a `base_update` checkpoint on top of the checkpoint last allowed, and the forge shows it with exactly two parents (that checkpoint, then the base's current head), the next call answers `base_updated`: it takes `ci -> ci`, the head moves to the update, the old allow is cleared and the review verdict stands. Nothing is allowed in that call; the call after it judges the new head. Parents that do not match refuse `base_update_parents_mismatch`. Any other head movement is `head_moved` as above.
