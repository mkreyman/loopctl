...
* a ranking change is an intentional, reviewed IMPROVEMENT — put the new numbers in their
  own commit ON TOP of the ranking commit in the same PR, never in the ranking commit
  itself, so the delta is visible in review without the change judging its own output (the
  tradeoff paragraph below has the full procedure).

Never re-baseline to make a red gate green without the reviewed decision recorded in the
PR. The whole point is that the numbers move only through a reviewed commit.
...
`--json` mirrors this: `baseline.status` narrows an otherwise-`ok` run carrying a
per-question verdict to `"question_regression"` or `"question_uncomparable"`, so a consumer
matching on the text-report statuses must handle those two as well.
...
worse; decide, in the PR, that the trade is worth it. The gate is RED at that point and is
meant to be — that red run IS the evidence, so let it FINISH and paste its per-question
table into the PR before pushing anything else: `cancel-in-progress` is on for non-master
refs (`.github/workflows/ci.yml:41-42`), so a second push kills the in-flight run and the
check history keeps a cancellation instead. Then push the re-baseline as its own reviewed
commit ON TOP of the ranking commit in the same PR, with that recorded decision as the
reason the gate is allowed to go green. Never in the SAME commit as the ranking change: a
baseline regenerated by the commit it is meant to judge compares the new code against its
own output. (A squash merge collapses the two on master — the separation that matters is
the reviewed decision in the PR, not the shape of the merged commit.) That is not
