...
- **Advisory locks were NOT in v1; they landed in Epic 40 (US-40.4) — still not a message type.** v1 was: post and read. The refinement shipped exactly as anticipated: an advisory FILE soft-lock is a plain post carrying the `claim:<target>` **key prefix**, a typed `refs` item (`{"type":"file","value":target}`), and a SHORT server-clamped TTL — no `kind` column, no new table. It is **advisory in the strict sense**: it never blocks a caller, it grants no exclusivity, and TWO sessions may hold a lock on the same file simultaneously (both are surfaced; the reading agent decides). Presence ("who's live", from `UpdateLastSeen`) is still a later refinement — see §7.
  - **Do not conflate it with the handoff claim.** `channel_claims` + `channel_claim`/`channel_release`/`channel_done` (US-40.B1) is the EXACTLY-ONCE primitive backed by a unique index. The soft-lock (`channel_lock`/`channel_unlock`/`channel_locks`) shares only a key-prefix convention with it — it is never the mechanism for "exactly one agent owns this work".

### 3.3 Read model — pull at the moments an agent can use it
- **SessionStart injection.** The existing recall-pack hook gains a **TEAM CHANNEL** section: active claims + recent posts from *other* sessions on this repo (self-deduped). A fresh session on any machine opens with "beelink, 20m ago: working X, opened PR #107, claimed `foo.ex`." This is the killer integration and it reuses machinery that already ships. Advisory locks are marked DISTINCTLY on that read (US-40.4): every `channel_recent` row carries `lock` (boolean), `lock_target` (the file) and `expires_at`, so a renderer can say "claimed: `foo.ex` by beelink, 4m ago" and tell a LIVE lock from an expired-but-unswept one without re-deriving the key convention itself. Only the newest few locks ride this read (the full live set is `channel_locks`), so lock refresh churn can never crowd genuine posts out of the newest-N window.
...
- **Epic 40 (US-40.4) — advisory file soft-locks:** `channel_lock(project, target, ttl_seconds?, note?)`, `channel_unlock(project, target)`, `channel_locks(project, limit?)` over `POST /api/v1/channel/locks`, `POST /api/v1/channel/locks/release`, `GET /api/v1/channel/locks`. Agent key. The WRITE path (`channel_lock`/`channel_unlock`) is project-scoped by the shared US-40.D3 membership gate; the READ (`channel_locks`) is **tenant-scoped only**, deliberately uniform with `channel_recent` — any agent in the tenant may read any of that tenant's channels' locks, and a foreign/nonexistent project returns an empty set rather than a 404 (oracle-safe). `channel_locks` is a SEPARATE, PINNED read — the read to TRUST for lock visibility, while `channel_recent` admits only the newest few locks so lock churn cannot crowd out real coordination posts (and locks it suppresses do NOT count toward its `has_more`, so never infer "nobody is editing this file" from `channel_recent`). `channel_locks` bounds a page two ways and REPORTS both, so a truncated page is never presented as a complete one: `meta.overflow` (the page cap) and `meta.holders_truncated` (the per-AGENT fairness cap — at most 20 rows per agent per page, partitioned on the server-stamped `agent_id` so a caller rotating the client-supplied `session_id` cannot escape it). Either flag true means live locks were dropped: raise `limit` or read the channel directly. `host`/`session_id` stay proxy-supplied: `session_id` is what makes a lock refreshable in place and releasable by slot, and a lock write **without** one is rejected (422) rather than rescued with a server-minted surrogate that could be neither refreshed nor released.
  - The TTL (`ttl_seconds`) is the ONE place `expires_at` is caller-influenced, and it is **server-clamped to [60, 3600] seconds** (default 900). The short TTL is driven by an INTERNAL marker that only the lock write path stamps — never by the `claim:` key prefix — so no request body can reach it: an ordinary post can never shorten or extend the uniform 30-day retention. The `claim:` key namespace is correspondingly **reserved**: `POST /api/v1/channel/posts` 422s an ordinary post that tries to use it (enforced by BOTH context writers, `Coordination.post/4` and `create_post/4`), which is what keeps `key LIKE 'claim:%'` a sound lock predicate for every read. The reservation is forward-looking, so every lock read ALSO carries a TTL-ceiling bound (the pinned read requires `expires_at <= now + 3600s`; the row-level `lock: true` marker requires `expires_at <= updated_at + 3600s`, anchored on `updated_at` so a refreshed lock — which preserves `inserted_at` — is still marked): a `claim:`-keyed row predating the reservation — or a lock quarantined and later released, which restores a longer deadline — carries the uniform 30-day retention and is therefore never published as a live file lock. A lock target is PATH-NORMALIZED (`./lib/foo.ex`, `lib//foo.ex`, `/lib/foo.ex` all address the same slot), so two sessions editing one file always collide on one target instead of holding invisible-to-each-other locks. A lock self-expires, so a crashed session can never hold a file: reads filter `expires_at > now` independently of the sweep, and `ChannelPostSweeper` reaps the row.
  - **Release scope is per-AGENT, not per-session.** The release slot is `(tenant, project, agent, session, key)`, but only `tenant` and `agent` are server-stamped; `session_id` is client-supplied and `channel_locks` publishes it for every live lock. Two sessions sharing one agent key can therefore release (or overwrite) each other's locks. Accepted for advisory hint data — nothing custody-bearing rides on a soft-lock — but never cite it as per-session isolation.
  - A release is audited under the distinct action `soft_lock_released`, NOT the `deleted` action the US-39.7 secret-redaction path uses, so routine lock churn cannot dilute that security signal.
- **later:** `presence(project)` (from the existing `UpdateLastSeen` plug) and a turn-boundary freshness hook (§5).
