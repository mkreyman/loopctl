...
- **Epic 40 (US-40.4) — advisory file soft-locks:** `channel_lock(project, target, ttl_seconds?, note?)`, `channel_unlock(project, target)`, `channel_locks(project, limit?)` over `POST /api/v1/channel/locks`, `POST /api/v1/channel/locks/release`, `GET /api/v1/channel/locks`. Agent key, project-scoped by the shared US-40.D3 membership gate. `channel_locks` is a SEPARATE, PINNED read — never truncated by `channel_recent`'s newest-N recency — so an active lock is always visible before a session edits. `host`/`session_id` stay proxy-supplied: `session_id` is what makes a lock refreshable in place and releasable by slot.
  - The TTL (`ttl_seconds`) is the ONE place `expires_at` is caller-influenced, and it is **server-clamped to [60, 3600] seconds** (default 900) on the `claim:`-keyed path only — an ordinary post can never shorten or extend the uniform 30-day retention. A lock therefore self-expires, so a crashed session can never hold a file: reads filter `expires_at > now` independently of the sweep, and `ChannelPostSweeper` reaps the row.
