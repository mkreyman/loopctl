...
| `Loopctl.Delivery.GitHubPullRequestSource` (`lib/loopctl/delivery/github_pull_request_source.ex`) | no — reads pull-request state, diffstat, changed names and a file tree | Operator-plane, fixed vendor host, GET only. The repository comes from the story's intake source, resolved server-side, so no tenant-supplied URL can reach it. Allowlisted. |
