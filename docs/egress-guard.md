...
| `Loopctl.Delivery.GitHubPullRequestSource` (`lib/loopctl/delivery/github_pull_request_source.ex`) | no — reads pull-request state, diffstat, changed names, a file tree and deployments; **also writes** a label, a comment and a close on the reporter's issue (#805) | Operator-plane, fixed vendor host. **Not GET only since #805** — that wording was true until this module became a writer, and the parity test compares module names, so nothing caught it. The repository comes from the story's intake source, resolved server-side, so no tenant-supplied URL can reach it. The writes carry no tenant and no reporter content: the comment body and the label are compile-time constants in `Loopctl.Delivery.Resolution`, never text from an intake record. Allowlisted. |
