...
1. **Novelty / dedup gate on create** — `Knowledge.propose_article/3` → the private `gate_proposal/4`
   (`propose_article/3` at `knowledge.ex:463`; the four `gate_proposal/4` clauses at `:473-525`).
   **SIX outcomes, not four** — `:duplicate`, `:low_novelty`, `:unknown`, `:novel`,
   ...
   `knowledge.ex:578-580`), and `:skipped_low_novelty` (`created: false`, `article` may be `nil`).
   ...
   is created but **forced to `status: "draft"`** (`:499`) with novelty stamped into
   `metadata.proposal_novelty` (`stamp_proposal_metadata/2`, `:699-712`) so a smarter consumer decides
   ...
   (`skip_low_novelty/4`, `:545-563`). That skip is decided LAST: an invalid `project_id`, an
   ...
   neighbor vanished between assess and now (`:479-487`), and `:unknown` creates only BY DEFAULT — a
   caller passing `on_gate_unavailable: :skip` gets `{:error, :gate_unavailable}` and nothing is
   created (`:514-520`). The assessor is config-injected (`Loopctl.Knowledge.ProposalGate`, `:469-471`)
   — do not hardcode it.
2. **Hybrid search provenance** — `Loopctl.Knowledge.hybrid_search/3` (`knowledge.ex:8471`).
...
5. **Heat must not rank on a signal heat produces** — `Knowledge.heat_index/2`
   (`knowledge.ex:9114`; the counted set is `@heat_read_access_types`, `:8982`). The heat index is the one retrieval route that
