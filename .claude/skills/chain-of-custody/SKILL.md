...
- **verify** — `validate_not_self_verify/4`. `nil` orchestrator identity is untrusted; a custody-orphaned story fails closed with
  ...
  `progress.ex:642-689`); that write is result-checked, and a failure flags `verifier_needed` plus a
  `verifier_not_assigned` audit event (`flag_verifier_needed/5`, `progress.ex:694`) instead of
...
(`progress.ex:450-472`) is what actually gates the custody ops: a `nil` `cap_id` is rejected with
`:missing_capability` **only for tenants that have an audit key** (`tenant_has_audit_key?/1`,
`progress.ex:722-727`); a pre-v2 (keyless) tenant returns `{:ok, :pre_v2_tenant}` and the operation
...
`assign_rotating_verifier/3` passes `[]` (`progress.ex:642-646`), and with `[]` the rejection is
