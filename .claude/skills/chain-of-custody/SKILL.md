...
- **verify** — `validate_not_self_verify/4`. `nil` orchestrator identity is untrusted; a custody-orphaned story fails closed with
  ...
  `progress.ex:714-761`); that write is result-checked, and a failure flags `verifier_needed` plus a
  `verifier_not_assigned` audit event (`flag_verifier_needed/5`, `progress.ex:766`) instead of
...
`progress.ex:794-799`); a pre-v2 (keyless) tenant returns `{:ok, :pre_v2_tenant}` and the operation
...
`assign_rotating_verifier/3` passes `[]` (`progress.ex:714-718`), and with `[]` the rejection is
