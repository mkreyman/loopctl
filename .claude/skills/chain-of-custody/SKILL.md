...
(`Dispatches.lineage_for_api_key/2`, `dispatches.ex:378-388`) — never read from the request body.
...
`lineage_shares_prefix?/2` (`dispatches.ex:399-403`) is the primitive the self-* checks use — note it
...
NOT share the implementer's root — rejected in SQL (`reject_same_root/2`, `dispatches.ex:470-476`)
