...
(`Dispatches.lineage_for_api_key/2`, `dispatches.ex:353-363`) — never read from the request body.
...
`lineage_shares_prefix?/2` (`dispatches.ex:374-378`) is the primitive the self-* checks use — note it
compares lineage **ROOTS** (element 0) only, not arbitrary prefixes, and an empty list on either side
is never a match. `select_verifier/3` (`dispatches.ex:400-437`) picks a verifier whose lineage does
NOT share the implementer's root — rejected in SQL (`reject_same_root/2`, `dispatches.ex:445-451`)
