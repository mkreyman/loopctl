...
6. **On `AdminRepo`/`HeavyReadRepo`, RLS does NOTHING — the explicit `tenant_id` predicate is the only
   ...
   (`heavy_read.ex:966-984`) RAISES unless EVERY base-table source — `from`, every join, and every
   ...
   `subject_id` equality on the outermost query (private `guard_memory!/3`, `heavy_read.ex:988-1010`), because `subject_id`
...
7. **Heavy reads can be SHED — handle `{:error, :heavy_read_overloaded}`.** `all/3` (`heavy_read.ex:671`),
   `one/3` (`heavy_read.ex:691`) and `all_memory/4` (`heavy_read.ex:727`) are specced to return it: the
   per-tenant cost-weighted in-flight gate (`gated/4`, `heavy_read.ex:756-776`) sheds over the cap —
...
transaction** (`heavy_read.ex:655-679` — `all/3` and `one/3`, both via `with_statement_timeout/5`;
