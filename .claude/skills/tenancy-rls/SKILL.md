...
6. **On `AdminRepo`/`HeavyReadRepo`, RLS does NOTHING — the explicit `tenant_id` predicate is the only
   ...
   (`heavy_read.ex:1525-1543`) RAISES unless EVERY base-table source — `from`, every join, and every
   ...
   Agent-memory reads need a SECOND predicate: `all_memory/4` (`:1198`) also requires a conjunctive
   `subject_id` equality on the outermost query (private `guard_memory!/3`, `heavy_read.ex:1547-1569`), because `subject_id`
...
7. **Heavy reads can be SHED — handle `{:error, :heavy_read_overloaded}`.** `all/3` (`heavy_read.ex:1139`),
   `one/3` (`heavy_read.ex:1161`) and `all_memory/4` (`heavy_read.ex:1198`) are specced to return it: the
   per-tenant cost-weighted in-flight gate (`gated/4`, `heavy_read.ex:1261-1267`) sheds over the cap —
   ...
   ids — must wrap both in `with_slot/3` (`heavy_read.ex:1245`). Acquiring per query releases the slot
...
transaction** (`all/3` at `heavy_read.ex:1139` and `one/3` at `:1161`, both via
`with_statement_timeout/5` at `:1272`;
