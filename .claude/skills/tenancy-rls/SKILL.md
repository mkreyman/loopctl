...
6. **On `AdminRepo`/`HeavyReadRepo`, RLS does NOTHING — the explicit `tenant_id` predicate is the only
   ...
   (`heavy_read.ex:1418-1436`) RAISES unless EVERY base-table source — `from`, every join, and every
   ...
   `subject_id` equality on the outermost query (private `guard_memory!/3`, `heavy_read.ex:1440-1472`), because `subject_id`
...
7. **Heavy reads can be SHED — handle `{:error, :heavy_read_overloaded}`.** `all/3` (`heavy_read.ex:1072`),
   `one/3` (`heavy_read.ex:1094`) and `all_memory/4` (`heavy_read.ex:1131`) are specced to return it: the
   per-tenant cost-weighted in-flight gate (`gated/4`, `heavy_read.ex:1194-1200`) sheds over the cap —
   ...
   ids — must wrap both in `with_slot/3` (`heavy_read.ex:1178`). Acquiring per query releases the slot
...
transaction** (`all/3` at `heavy_read.ex:1072` and `one/3` at `:1094`, both via
`with_statement_timeout/5` at `:1202`;
