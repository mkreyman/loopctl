...
6. **On `AdminRepo`/`HeavyReadRepo`, RLS does NOTHING — the explicit `tenant_id` predicate is the only
   ...
   (`heavy_read.ex:1376-1394`) RAISES unless EVERY base-table source — `from`, every join, and every
   ...
   `subject_id` equality on the outermost query (private `guard_memory!/3`, `heavy_read.ex:1398-1430`), because `subject_id`
...
7. **Heavy reads can be SHED — handle `{:error, :heavy_read_overloaded}`.** `all/3` (`heavy_read.ex:1034`),
   `one/3` (`heavy_read.ex:1054`) and `all_memory/4` (`heavy_read.ex:1090`) are specced to return it: the
   per-tenant cost-weighted in-flight gate (`gated/4`, `heavy_read.ex:1152-1158`) sheds over the cap —
   ...
   ids — must wrap both in `with_slot/3` (`heavy_read.ex:1136`). Acquiring per query releases the slot
...
transaction** (`all/3` at `heavy_read.ex:1034` and `one/3` at `:1054`, both via
